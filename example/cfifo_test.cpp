...
void test_multiple_cursors_read_same_messages()
{
    ...
    assert(cursor_40_a.sequenceID == 0);
    assert(message == "A");
    ...
    assert(cursor_40_b.sequenceID == 1);
    assert(message == "B");
    ...
    assert(cursor_41_a.sequenceID == 0);
    assert(message == "A");
    ...
    assert(cursor_41_b.sequenceID == 1);
    assert(message == "B");
...
void test_write_credit_and_no_cursor_reclaim()
{
    ...
    assert(overflow.status == CFIFOWriteStatus::SUCCESS);
...
void test_reclaim_reports_skipped_messages()
{
    ...
    assert(after_reclaim.sequenceID == 2);
    assert(message == "C");
...
void test_reclaim_does_not_treat_subscriber_id_as_sequence()
{
    ...
    assert(write.status == CFIFOWriteStatus::SUCCESS);
    assert(queue.size() == 1);
    assert(queue.credit() == 1);
...
void test_reclaim_tied_slowest_cursors_preserves_advanced_cursor()
{
    ...
    assert(advanced.movedBy == 0);
    assert(advanced.pendingMessage == 2);
    assert(message == "B");
